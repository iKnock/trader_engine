    ...
    for i in range(1, len(ohlc_dict)):
        if tickers_signal == "":
            ...
            if ohlc_dict["LOW"][i] < ohlc_dict["CLOSE"][i - 1] - ohlc_dict["ATR"][i - 1]:
                tickers_signal = ""
                df_signal.append("Sell/Stop Loss")#selling price will be ohlc_dict["CLOSE"][i - 1] - ohlc_dict["ATR"][i - 1]
                tickers_ret.append(
...
if __name__ == '__main__':
    df_ind = run()
    df_for_breakout = df_ind.iloc[:, [0, 1, 2, 3, 4, 7, 14, 15, 16]]
    break_df = breakout(df_for_breakout)
